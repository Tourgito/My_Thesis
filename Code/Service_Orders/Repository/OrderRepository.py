...
class OrderRepository(object):
      ...
      def __init__(self):
          ...
          self.session = self.cluster.connect('orderservice')
      ...
      def UpdateEventThatItHasBeenPublished(self,EntityId,EntityType,EventTime,EventType):
           self.session.execute("UPDATE Events SET Published = True WHERE EntityId=%s and EntityType=%s and EventTime=%s and EventType = %s", [EntityId,EntityType,EventTime,EventType])
